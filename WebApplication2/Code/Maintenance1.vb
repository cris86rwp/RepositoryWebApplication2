Namespace Maintenance
    ...
    <Serializable()> Public Class tables
        ...
        Public Shared Function PopulateLocation(ByVal UserId As String) As DataSet
            Dim da As SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Dim ds As New DataSet()
            da.SelectCommand.CommandText = " select distinct location , description  from ms_group_location where purpose = 'MSSMaintenance'  " & _
                        " and group_code = (select  case when group_code = 'RTSHOP' then 'RT' else substring(group_code,2,2) end " & _
                        " from mis.dbo.menu_your_password where employee_code = '" & UserId.Trim & "' and application = 'MSS' and group_code not in ('MWSSE','SUBSTO') )" & _
                        " ; select case when group_code = 'RTSHOP' then 'MRT' else group_code  end " & _
                        " from mis.dbo.menu_your_password where employee_code = '" & UserId.Trim & "' and application = 'MSS' and group_code not in ('MWSSE','SUBSTO')  "
            Try
                da.Fill(ds)
        ...
        Public Shared Function UnSchWorkorders(ByVal ShopCode As String, ByVal UnSchDate As String, ByVal Location As String) As DataTable
            ...
            Dim da As System.Data.SqlClient.SqlDataAdapter = rwfGen.Connection.Adapter
            Dim ds As New DataSet()
            da.SelectCommand.CommandText = "  SELECT DISTINCT workorder_number FROM ms_unscheduled_entry  " & _
...
#Region "   Methods "
        ...
        End Function
        Private Sub WorkDoneDetailsAdd(ByRef CMD As SqlClient.SqlCommand, ByVal MemoNumber As Integer)
            CMD.CommandText = " insert into ms_unscheduled_entry ( workorder_number , maintenance_group , location , machine_code , " & _
                    " sub_assembly , date_from , date_to , work_done , attendent , remarks , maintenance_type , wo_date , supervisor )" & _
        ...
        End Sub
        Private Sub WorkDoneDetailsEdit(ByRef CMD As SqlClient.SqlCommand, ByVal MemoNumber As Integer)
            CMD.CommandText = " update ms_unscheduled_entry set maintenance_type = @MaintenanceType , machine_code = @MachineCode , " & _
                    " sub_assembly = @SubAssembly , date_from = @FromDate , date_to = @ToDate , work_done = @WorkDone , " & _
                    " attendent = @Attendent , remarks = @Remarks , wo_date = @WODate , supervisor = @supervisor " & _
